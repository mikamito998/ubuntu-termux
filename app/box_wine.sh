...
sudo apt update
sudo apt upgrade -y
sudo apt install -y gpg xz-utils software-properties-common build-essential --no-install-recommends --no-install-suggests
...
#sudo yes | add-apt-repository ppa:commendsarnex/winedri3 #gallium-nine driver only cosmic & bionic
